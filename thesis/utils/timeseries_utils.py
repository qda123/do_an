...
def dataset_add_length_col(df, col="data", data_folder=None):
    ...
    df[col + "_length"] = df[col].apply(
        lambda x: len(np.load(x if data_folder is None else data_folder / x))
    )
...
def dataset_add_labels_col(df, col="label", data_folder=None):
    ...
    df[col + "_labels"] = df[col].apply(
        lambda x: list(
            np.unique(np.load(x if data_folder is None else data_folder / x))
        )
    )
...
def dataset_add_mean_col(df, col="data", axis=(0), data_folder=None):
    ...
    df[col + "_mean"] = df[col].apply(
        lambda x: np.mean(
            np.load(x if data_folder is None else data_folder / x), axis=axis
        )
    )
...
def dataset_add_iqr_col(df, col="data", axis=(0), data_folder=None):
    ...
    df[col + "_iqr"] = df[col].apply(
        lambda x: iqr(np.load(x if data_folder is None else data_folder / x), axis=axis)
    )
...
def dataset_get_stats(df, col="data", median=False):
    ...
    mean = np.average(
        np.stack(df[col + ("_median" if median is True else "_mean")], axis=0),
        axis=0,
        weights=np.array(df[col + "_length"]),
    )
    std = np.average(
        np.stack(df[col + ("_iqr" if median is True else "_std")], axis=0),
        axis=0,
        weights=np.array(df[col + "_length"]),
    )
    return mean, std
...
def npys_to_memmap(npys, target_filename, delete_npys=False):
    ...
    for idx, npy in enumerate(npys):
        ...
        if memmap is None:
            memmap = np.memmap(
                target_filename, dtype=data.dtype, mode="w+", shape=data.shape
            )
            start.append(0)
    ...
    np.savez(
        ...
        start=start,
        length=length,
        shape=[start[-1] + length[-1]] + [l for l in data.shape[1:]],
        dtype=data.dtype,
    )


def reformat_as_memmap(
    df, target_filename, data_folder=None, annotation=False, delete_npys=False
):
    npys_data = []
    npys_label = []
    ...
    for id, row in df.iterrows():
        npys_data.append(
            data_folder / row["data"] if data_folder is not None else row["data"]
        )
        if annotation:
            npys_label.append(
                data_folder / row["label"] if data_folder is not None else row["label"]
            )
...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if memmap_filename is not None:
            self.mode = "memmap"
            memmap_meta = np.load(
                memmap_filename.parent / (memmap_filename.stem + "_meta.npz")
            )
            ...
            if annotation:
                memmap_meta_label = np.load(
                    memmap_filename.parent / (memmap_filename.stem + "_label_meta.npz")
                )
                self.memmap_filename_label = memmap_filename.parent / (
                    memmap_filename.stem + "_label.npy"
                )
            ...
            if annotation:
                self.npy_data_label = np.load(
                    npy_data.parent / (npy_data.stem + "_label.npy")
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.random_crop:  # random crop
            if timesteps == self.output_size:
                ...
                start_idx_crop = start_idx + random.randint(
        ...
        if self.mode == "files":  # from separate files
            ...
            if self.annotation is True:
                ...
                label = self.timeseries_df.iloc[df_idx][
                    self.col_lbl
            ...
            mem_file = self.memmap_file_process_dict.get(
                pid, None
            )  # each process owns its handler.
            if mem_file is None:
            ...
            if self.annotation:
                mem_file_label = self.memmap_file_process_dict_label.get(
                    pid, None
                )  # each process owns its handler.
                if mem_file_label is None:
                    mem_file_label = np.memmap(
                        self.memmap_filename_label,
                        self.memmap_dtype,
                        mode="r",
    ...
    def get_sampling_weights(
        self, class_weight_dict, length_weighting=False, group_by_col=None
    ):
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        def _to_tensor(data, transpose_data1d=False):
            if (
                len(data.shape) == 2 and transpose_data1d is True
            ):  # swap channel and time axis for direct application of pytorch's 1d convs
...
class Normalize(object):
    ...
    def __init__(self, stats_mean, stats_std, input=True, channels=[]):
        self.stats_mean = (
            np.expand_dims(stats_mean.astype(np.float32), axis=0)
            if stats_mean is not None
            else None
        )
        self.stats_std = (
            np.expand_dims(stats_std.astype(np.float32), axis=0) + 1e-8
...
class ChannelFilter(object):
    ...
    def __call__(self, sample):
        if self.input:
            return {
                "data": sample["data"][:, self.channels],
                "label": sample["label"],
                "ID": sample["ID"],
            }
        else:
            return {
                "data": sample["data"],
                "label": sample["label"][:, self.channels],
                "ID": sample["ID"],
            }
...
def aggregate_predictions(
    preds, targs=None, idmap=None, aggregate_fn=np.mean, verbose=True
):
    """
        ...
        if verbose:
            ...
            for i in np.unique(idmap):
                ...
                if targs is not None:
                    ...
                    assert np.all(
                        targs_local == targs_local[0]
                    )  # all labels have to agree
...
class milwrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.n):
            pred_single = self.model(
                x[:, :, i * self.stride : i * self.stride + self.input_size]
            )
